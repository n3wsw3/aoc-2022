...
fn parse_sections(value: &str) -> (u32, u32) {
    value.split("-").map(|val| val.parse::<u32>().unwrap()).collect_tuple().unwrap()
}
...
pub fn part_one(input: &str) -> Option<u32> {
    Some(parse_pairs(input).into_iter().map(|(first, last)| if section_contains(first, last) {1} else {0}).sum())
}
...
pub fn part_two(input: &str) -> Option<u32> {
    Some(parse_pairs(input).into_iter().map(|(first, last)| if section_contains2(first, last) {1} else {0}).sum())
}
